...
///
/// The Search API includes two different endpoints: `/suggest` and `/retrieve`. As the user types, text is sent to the `/suggest` endpoint to get suggested results. When the user selects a suggestion, its `mapbox_id` is sent to `/retrieve` to get the full data.
class SearchAPI {
  ...
  Future<ApiResponse<SuggestionResponse>> getSuggestions(
    ...
    debugPrint('Request to ${uri.toString()}');
  ...
  Future<ApiResponse<RetrieveResponse>> getPlace(String mapboxId) async {
    ...
    if (response.statusCode != 200) {
      ...
      return ApiResponse(
        success: RetrieveResponse.fromJson(json.decode(response.body)),
      );
    }
  }
}
